...
export const acidtankmixed = async (c) => {

  const timeoutPromise = new Promise((_, reject) => {
    setTimeout(() => reject(new Error("Request timed out")), timeout);
  });

  const acidtankmixedDataListLogic = new Promise(async (resolve, reject) => {
    try {
      // ใส่ logic การทำงานของคุณตรงนี้
      // เช่น การเรียกดูข้อมูลจากฐานข้อมูล
      const timestamp = await convertTotimestamp(c.req.query()); // แปลง timestamp
  ...
  try {
    const result = await Promise.race([acidtankmixedDataListLogic, timeoutPromise]);
...
// Dashboard HCI Tank Store
export const acidtankstore = async (c) => {

  const timeoutPromise = new Promise((_, reject) => {
    setTimeout(() => reject(new Error("Request timed out")), timeout);
  });

  const acidtankstoreDataListLogic = new Promise(async (resolve, reject) => {
    try {
      // ใส่ logic การทำงานของคุณตรงนี้
      // เช่น การเรียกดูข้อมูลจากฐานข้อมูล
      const timestamp = await convertTotimestamp(c.req.query()); // แปลง timestamp
  ...
  try {
    const result = await Promise.race([acidtankstoreDataListLogic, timeoutPromise]);
